...
const ClassroomManagementPage: React.FC = () => {
  ...
  const { data: studentsWithoutClassroomData, refetch: refetchStudents } =
    useQuery<StudentsWithoutClassroomData>(GET_STUDENTS_WITHOUT_CLASSROOM);
  const { data: allStudentsData, refetch: refetchAllStudents } =
    useQuery<AllStudentsData>(GET_ALL_STUDENTS);
  const {
  ...
  const filteredClassrooms = useMemo(() => {
    ...
    return classroomsData.classrooms.filter(
      (classroom: any) =>
    ...
    onCompleted: () => {
      ...
      refetchClassrooms().then((result: any) => {
        ...
        const updatedClassroom = classrooms.find(
          (c: any) =>
            Number(c.classroomid) === Number(selectedClassroom?.classroomid)
        );
  ...
  const handleAddStudent = (studentid: string | number) => {
    ...
    const classroomIdNumber = parseInt(
    ...
    if (
      ...
      const studentToAdd =
        studentsWithoutClassroomData.studentsWithoutClassroom.find(
  ...
  const handleDelete = (classroomid: number) => {
    ...
    modal.confirm({
      ...
      onOk() {
        deleteClassroom({
          variables: { classroomid: parseInt(classroomid.toString(), 10) },
        });
      },
    });
  };
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              render: (text: any, record: any) => (
                <Button
                  onClick={() => handleRemoveStudent(record.studentid)}
                  danger
                >
                  Remove
                </Button>
          ...
          options={studentsWithoutClassroomData?.studentsWithoutClassroom
            ?.filter((student) => {
              ...
              const inAnyClassroom = allStudentsData?.students?.some(
                (s) =>
                  Number(s.studentid) === Number(student.studentid) &&
                  s.classrooms &&
