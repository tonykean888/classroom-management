...
const StudentManagementPage: React.FC = () => {
  ...
  const { loading: gradelevelsLoading, data: gradelevelsData } =
    useQuery(GET_GRADELEVELS);
  const { loading: gendersLoading, data: gendersData } = useQuery(GET_GENDERS);
  const { loading: prefixesLoading, data: prefixesData } =
    useQuery(GET_PREFIXES);
  ...
  const [deleteStudent] = useMutation(DELETE_STUDENT, {
    ...
    update: (cache, { data }, { variables }) => {
      console.log(
        'Update cache after deletion, data:',
        data,
        'variables:',
        variables
      );
        ...
        cache.modify({
          fields: {
            students(existingStudents = [], { readField }) {
              return existingStudents.filter((studentRef: any) => {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      fetchStudents({
        variables: { search: searchValue, gradelevelid: selectedGradelevel },
      });
    }, 800); // 800ms delay
  ...
  const showModal = (student: any = null) => {
    ...
    if (student) {
      ...
      form.setFieldsValue({
        ...
        gradelevelid: student.gradelevel
          ? student.gradelevel.gradelevelid
          : null,
  ...
  const onFinish = (values: any) => {
    const input = {
      ...
      gradelevelid: values.gradelevelid
        ? parseInt(values.gradelevelid, 10)
        : undefined,
      ...
      render: (text: any, record: any) => (
        <Space>
          ...
          <Button
            ...
            onClick={() => {
              console.log(
                'Delete button clicked for student ID:',
                record.studentid
              );
  ...
  return (
    <div>
      ...
      <Table
        dataSource={(studentsData as any)?.students || []}
        columns={columns}
        rowKey="studentid"
