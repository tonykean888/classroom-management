...
export class ClassroomService {
  ...
  async delete(classroomid: number): Promise<Classroom> {
	...
	async addStudentToClassroom(classroomid: number, studentid: number): Promise<Classroom> {
	...
	async removeStudentFromClassroom(classroomid: number, studentid: number): Promise<Classroom> {
    ...
    try {
      ...
      const studentClassrooms = await this.studentClassroomRepository.find({
        ...
        relations: ['student', 'student.gender', 'student.prefix', 'student.gradelevel'],
      });
      
      // Extract the student objects from the results
      const students = studentClassrooms.map(sc => sc.student);
      console.log(`Found ${students.length} students in classroom ${classroomid}`);
