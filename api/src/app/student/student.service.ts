...
export class StudentService {
    ...
    student.birthdate = input.birthdate;
		student.gender = (await this.genderRepository.findOneBy({ genderid: input.genderid })) ?? undefined;
    student.prefix = (await this.prefixRepository.findOneBy({ prefixid: input.prefixid })) ?? undefined;
    student.gradelevel = (await this.gradelevelRepository.findOneBy({ gradelevelid: input.gradelevelid })) ?? undefined;
  ...
  async delete(studentid: number): Promise<boolean> {
    try {
      console.log('Service attempting to delete student with ID:', studentid, 'type:', typeof studentid);
